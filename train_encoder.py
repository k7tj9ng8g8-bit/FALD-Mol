...
class vae_encoder(pl.LightningModule):
    def __init__(self, tokenizer=None, config=None, loader_len=0, no_train=False):
        ...
        for p in self.text_proj_m.parameters():
            p.requires_grad = False
    ...
    def forward(self, text1_input_ids, text1_attention_mask, text2_input_ids, text2_attention_mask, alpha=0):
        ...
        text1_embeds = self.text_encoder(text1_input_ids, attention_mask=text1_attention_mask,
                                         return_dict=True).last_hidden_state
        text1_feat = F.normalize(self.text_proj(text1_embeds[:, 0, :]), dim=-1)
        text2_embeds = self.text_encoder(text2_input_ids, attention_mask=text2_attention_mask,
                                         return_dict=True).last_hidden_state
        text2_feat = F.normalize(self.text_proj(text2_embeds[:, 0, :]), dim=-1)

        with torch.no_grad():
            self._momentum_update()
            text1_embeds_m = self.text_encoder_m(text1_input_ids, attention_mask=text1_attention_mask,
                                                 return_dict=True).last_hidden_state
            text1_feat_m = F.normalize(self.text_proj_m(text1_embeds_m[:, 0, :]), dim=-1)
            text1_feat_all = torch.cat([text1_feat_m.t(), self.text1_queue.clone().detach()], dim=1)

            text2_embeds_m = self.text_encoder_m(text2_input_ids, attention_mask=text2_attention_mask,
                                                 return_dict=True).last_hidden_state
            text2_feat_m = F.normalize(self.text_proj_m(text2_embeds_m[:, 0, :]), dim=-1)
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        for t in tmp:
            try:
                t2 = '[CLS]' + Chem.MolToSmiles(self.aug([Chem.MolFromSmiles(t[5:])])[0], canonical=False,
                                                isomericSmiles=True)
                text1.append(t)
                text2.append(t2)
            except:
...
def main(args, config):
    ...
    dataset = SMILESDataset_pretrain(args.data_path, data_length=None)
    print('#data:', len(dataset))
    data_loader = DataLoader(dataset, batch_size=config['batch_size'], num_workers=4, shuffle=False, pin_memory=True,
                             drop_last=True)
    tokenizer = regexTokenizer(vocab_path=args.vocab_filename, max_len=127)

    print("Creating model")
    ngpu = 1
    model = vae_encoder(config=config, tokenizer=tokenizer, loader_len=len(data_loader) // ngpu)
...
def evaluate(args, config):
    print("Creating dataset")
    dataset = SMILESDataset_pretrain(args.data_path, data_length=[0, 10], is_train=False, norm_path='D:\project\FALD-Mol\SPMM\normalize.pkl')
    print('#data:', len(dataset))
    data_loader = DataLoader(dataset, batch_size=config['batch_size'], num_workers=8, shuffle=True, pin_memory=True,
                             drop_last=False)
    tokenizer = regexTokenizer(vocab_path=args.vocab_filename, max_len=127)
    ...
    for text1 in data_loader:
        ...
        text1_sc = [Chem.MolToSmiles(random.choice(list(EnumerateStereoisomers(Chem.MolFromSmiles(t)))), canonical=True,
                                     isomericSmiles=True) for t in text1]
        text1 = [Chem.MolToSmiles(random.choice(list(EnumerateStereoisomers(Chem.MolFromSmiles(t)))), canonical=True,
                                  isomericSmiles=True) for t in text1]
        text1_combined = []
        ...
        text2 = [
            '[CLS]' + Chem.MolToSmiles(model.aug([Chem.MolFromSmiles(t[5:])])[0], canonical=False, isomericSmiles=True)
            for t in text1]
        ...
        text1_embeds = model.text_encoder(text_input_ids, attention_mask=text_attention_mask,
                                          return_dict=True).last_hidden_state
        text1_feat = F.normalize(model.text_proj(text1_embeds[:, 0, :]), dim=-1)
        text2_embeds = model.text_encoder(text2_input_ids, attention_mask=text2_attention_mask,
                                          return_dict=True).last_hidden_state
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='D:\project\FALD-Mol\data\unlabeled_200k.txt')
    parser.add_argument('--resume', default=False, type=bool)
    parser.add_argument('--output_dir', default='D:\project\FALD-Mol\Pretrain_encoder')
